...
### Step 17: 系统设置页 — 样式表路径选择器

| # | 操作 | 文件 | 说明 |
|---|------|------|------|
| 17.1 | `initializeSchema` 追加 `CREATE TABLE IF NOT EXISTS settings(key TEXT PRIMARY KEY, value TEXT)` | `DataCenter/RecordRepository.cpp:14` | 持久化存储设置项 |
| 17.2 | 新增 `getSetting(key)` / `setSetting(key, value)` | `DataCenter/RecordRepository.h/.cpp` | settings 表 CRUD |
| 17.3 | DataCenter 新增 `getSetting` / `setSetting` 门面 | `DataCenter/DataCenter.h/.cpp` | 业务层委托 |
| 17.4 | 替换设置页占位标签 → 完整表单（路径输入框 + 浏览/保存/恢复按钮） | `MyWindow/MainWindow.cpp:167-173` | UI 实现 |
| 17.5 | 浏览按钮打开 `QFileDialog` 选取 `.css` 文件，填入路径框 | `MyWindow/MainWindow.cpp` | 交互逻辑 |
| 17.6 | 保存并应用：路径写入 settings 表 → `QFile` 读取 → `qApp->setStyleSheet()` | `MyWindow/MainWindow.cpp` | 交互逻辑 |
| 17.7 | 恢复默认：清空 `style_path` 设置 → 重载 `{appDir}/config/style.css` | `MyWindow/MainWindow.cpp` | 交互逻辑 |
| 17.8 | 启动时从 DB 加载自定义样式：`initTables()` 后检查 `style_path` 设置 | `MyWindow/MainWindow.cpp` | 启动自动恢复 |
...
| 6 (Step 13) | DataCenter 安全加固（P0-S1, P0-S2） | DONE |
| 6 (Step 14) | SQL 安全加固（P1-H3, P2-A1/A2/A3） | DONE |
| 6 (Step 15) | UI 层配置去硬编码（P1-H1/H2, P2-A4） | DONE |
| 6 (Step 16) | 抽取 Export.h（P2-A5） | DONE |
| 7 (Step 17) | 系统设置页 — 样式表路径选择器 | PENDING |